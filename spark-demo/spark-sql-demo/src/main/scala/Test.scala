...
object Test {
    ...
    def main(args: Array[String]): Unit = {
        val spark = SparkSession.builder()
            ...
            .config("mapreduce.input.fileinputformat.split.minsize", 134217728)
            .config("spark.sql.adaptive.enabled", "true")
            .config("spark.sql.adaptive.shuffle.targetPostShuffleInputSize", 134217728 / 2)
            .appName("dd")
            //            .enableHiveSupport()
            .getOrCreate()
        val courseShoppingCartDF = spark.read.json("D:\\培训\\阿里云项目\\01日志数据\\jjj")
        courseShoppingCartDF.createOrReplaceTempView("course_pay")

        courseShoppingCartDF.printSchema()
        val sql =
            """
              |select orderid,paymoney,discount,createtime,dt,dn from course_pay
              |distribute by cast(rand() * 3 as int)
            """.stripMargin


        val df = spark.sql(sql)
        df.cache()
        df.write.partitionBy("dt", "dn").saveAsTable("course_pay")
